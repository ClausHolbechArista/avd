...
class RouterBgpMixin(UtilsMixin):
    ...
    def _router_bgp_peer_groups(self) -> dict | None:
        ...
        if (self._vrf_default_ipv4_subnets or self._vrf_default_ipv4_static_routes) and self._vtep and self._evpn_role in ["client", "server"]:
    ...
    def _router_bgp_vrfs(self) -> dict | None:
        ...
        """
        if not (self._vtep or self._mpls_ler):
            return None
        ...
        for tenant in self._filtered_tenants:
            for vrf in tenant["vrfs"]:
                ...
                if vrf_name == "default" and self._evpn_role in ["client", "server"] and self._vrf_default_ipv4_subnets:
    ...
    def _router_bgp_vlans(self) -> dict | None:
        ...
        if not (
            ...
            and (self._vtep or self._mpls_ler)
            and (self._evpn_role in ["client", "server"] or self._mpls_overlay_role in ["client", "server"])
        ):
            return None
    ...
    def _router_bgp_vlan_aware_bundles(self) -> dict | None:
        ...
        if not (self._network_services_l2 and self._evpn_role in ["client", "server"] and "evpn" in self._overlay_address_families):
            return None
    ...
    def _router_bgp_vpws(self) -> list[dict] | None:
        ...
        if not (self._network_services_l1 and self._mpls_ler and self._mpls_overlay_role):
            return None
