# Network Services Variables for mpls-vpn Design - VRFs/VLANs
...
tenants:
  ...
  < tenant_a >:
    ...
    # e.g. mac_vrf_rt_base = 10000, svi 100 = VNI 10100, svi 300 = VNI 10300.
    mac_vrf_rt_base: < 10000-16770000 >

    # Base number for vlan_aware_bundle | Optional.
    # The "Assigned Number" part of RD/RT is derived from vrf_id + vlan_aware_bundle_number_base.
    ...
    vrfs:
      ...
      < tenant_a_vrf_1 >:
        ...
        # VRF VNI | Required.
        # The VRF VNI range is not limited, but it is recommended to keep vrf_id <= 1024
        # If vrf_id > 10000 make sure to adjust mac_vrf_rt_base accordingly to avoid overlap.
        vrf_id: < 1-1024 >
        ...
        svis:
          ...
          < 1-4096 >:
            ...
            # By default the vni will be derived from "mac_vrf_rt_base:"
    ...
    # Pseudowire rt base, used to generate route targets for vpws services. Recommended values max vrf_id < pw_rt_base < min mac_vrf_rt_base.
    pseudowire_rt_base: < int >

    # Define L1 network services (pseudowires) as a list of dictionaries.
    l1_services:
      ...
      - name: < name >
        ...
        # Local and remote IDs for pseudowire
        ids: [ < node1 local id >, < node2 local id > ]

        # Interfaces patched to the pseudowire on both sides.
        # If interfaces are subinterfaces, pseudowires will run in "VLAN mode" and L2 subinterfaces will be rendered.
        # Otherwise the pseudowire runs in "port mode".
