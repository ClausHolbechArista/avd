...
description:
  - The `arista.avd.eos_designs_facts` module is an Ansible Action Plugin setting the following ansible_facts:
  - `avd_switch_facts`: Dictionary containing `switch` facts per host.
  - `avd_topology_peers`: Dictionary containing list of downlink switches per host.
    This list is built based on the `uplink_switches` from all other hosts.
  - `avd_overlay_peers`: Dictionary containing list of EVPN or MPLS overlay peers per host.
  ...
  - The module is used in `arista.avd.eos_designs` to set facts for devices, which are then used by jinja templates
    and python modules in `arista.avd.eos_designs` to generate the `structured_configuration`.
options:
  schema:
